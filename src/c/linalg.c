...
static int strsm(const char* side, const char* uplo, const char* transa,
                 const char* diag, int* m, int* n, float* alpha,
                 const float* a, int* lda, float* b, int* ldb);
static int sgemm(char* transa, char* transb, int* m, int* n, int* k, float* alpha,
                 float* a, int* lda, float* b, int* ldb, float* beta,
                 float* c__, int* ldc);
static int ssyrk(char* uplo, char* trans, int* n, int* k,
                 float* alpha, float* a, int* lda, float* beta, float* c__, int* ldc);
static int ssymm(char* side, char* uplo, int* m, int* n,
            ...
            const float* B, float beta, float* C)
{
    ...
    int result = sgemm((char*)ta,(char*)tb,&n,&k,&m,&alpha,(float*)A,&lda,(float*)B,&ldb,&beta,C,&n);
    assert(result == 0);
}
...
void trisolveright(const float* L, float* A,
