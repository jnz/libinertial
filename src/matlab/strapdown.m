...
% @param[in] dt_sec 1x1 Time interval between epoch k and k+1 [s]
% @param[in] specific_force_b 3x1 specific force measurement in epoch k in body frame (NED) [m/s/s]
% @param[in] omega_b_ib 3x1 rotation rate of body relative to n-frame, measurement in epoch k in body frame coordinates (NED) [rad/s]
% @param[in] qk 4x1 orientation quaternion from body to n-frame (NED) in epoch k
% @param[in] vel_n_k 3x1 velocity of body relative to Earth in n-frame (NED) at epoch k [m/s]
% @param[in] latitude_rad 1x1 latitude on ellipsoid. Must be less than |pi/2| as tan(latitude_rad) is calculated [rad]
% @param[in] height_m 1x1 altitude/height above ellipsoid [m]
